...
public class Element implements IElement
{
     ...
     * 
     * @param locator
     *            a By which defines this element by its first match
     */
    ...
    public static void waitForAnyElementPresentOf(long time, Element... elements) throws WidgetException
    {
        String locator = "";
    ...
    public void waitForAttributeEqualTo(final String attributeName, final String attributeValue, long timeout) throws WidgetTimeoutException
    {
        waitForCommand(new ITimerCallback()
        {
            ...
            public String toString()
            {
                return "Waiting for attribute, " + ((attributeName == null) ? "" : attributeName) + ", to be equal to: "
                        + ((attributeValue == null) ? "" : attributeValue) + " - for the element with the locator: " + locator;
            }
        }, timeout);
    ...
    public void waitForAttributeNotEqualTo(final String attributeName, final String attributeValue, long timeout) throws WidgetTimeoutException
    {
        waitForCommand(new ITimerCallback()
        {
            ...
            public String toString()
            {
                return "Waiting for attribute, " + ((attributeName == null) ? "" : attributeName) + ", to not equal: "
                        + ((attributeValue == null) ? "" : attributeValue) + " - for the element with the locator: " + locator;
            }
        }, timeout);
    ...
    public void fireEvent(String event) throws WidgetException
    {
        try
        {
            ...
            {
                javaScript =
                        "if(document.createEvent){var evObj = document.createEvent('MouseEvents');evObj.initEvent('" + event
            ...
            {
                javaScript =
                        "if(document.createEvent){var evObj = document.createEvent('HTMLEvents');evObj.initEvent('" + event
                                + "', true, false); arguments[0].dispatchEvent(evObj);} else if(document.createEventObject) { arguments[0].fireEvent('on"
                                + event + "');}";
            }
            eval(javaScript);
